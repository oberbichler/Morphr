...
class RotationCoupling(eq.Objective):
    def __init__(self, nodes_a, nodes_b, shape_functions_a, shape_functions_b, rotation_axis, weight):
    ...
    def compute(self, g, h):
        ...
        angle_a = np.arcsin(np.dot(omega_a, self.rotation_axis))
        angle_b = np.arcsin(np.dot(omega_b, self.rotation_axis))
